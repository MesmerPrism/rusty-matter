...
The generated Rust module stores a normalized ``TriangleMeshSurface`` as plain
positions and triangle indices. GLB remains an external/provenance source; core
Matter crates consume only their own mesh contract.
"""
...
def write_rust_module(
    ...
    lines: list[str] = [
        ...
        "",
        f"#[rustfmt::skip]\nconst PLANARIA_SKETCHFAB_POSITIONS: [[f32; 3]; {len(positions)}] = [",
    ]
    lines.extend(f"    {format_triplet(position)}," for position in positions)
    lines.extend(
        [
            "];",
            "",
            f"#[rustfmt::skip]\nconst PLANARIA_SKETCHFAB_TRIANGLES: [[u32; 3]; {len(triangles)}] = [",
        ]
    )
    ...
    lines.extend(
        [
            ...
            "        PLANARIA_SKETCHFAB_SURFACE_ID,",
            "        PLANARIA_SKETCHFAB_POSITIONS",
            "            .iter()",
            "            .map(|position| Vec3::new(position[0], position[1], position[2]))",
            "            .collect(),",
            "        PLANARIA_SKETCHFAB_TRIANGLES.to_vec(),",
            "    )",
            "}",
            "",
        ]
